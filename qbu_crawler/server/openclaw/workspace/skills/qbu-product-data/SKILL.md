...
- `text_review_total_current`：站点展示评论总数扣除 `ratings_only_count` 后的可采集文字评论总数
- `ratings_only_count`：站点展示评论中只有星级、没有文字正文的数量；这类评论通常无法抓取到正文

禁止把 `ingested_review_rows` 和 `site_reported_review_total_current` 混称为同一个“评论数”。分析时必须显式说明当前采用的口径。
覆盖率分析必须区分：

- 业务样本覆盖率：`ingested_review_rows / site_reported_review_total_current`
- 爬虫采集完整率：`ingested_review_rows / (site_reported_review_total_current - ratings_only_count)`

## Time Axis Rules

- `product_state_time`：产品状态更新时间，适用于价格、库存、评分、站点展示评论总数等当前态信息
- `review_ingest_time`：评论抓取入库时间，适用于“最近抓到什么”“最近新增了多少评论”
- `review_publish_time`：评论在站点上的发布时间，适用于“某时间段用户说了什么”，SQL 优先使用 `date_published_parsed`
...
```sql
SELECT r.rating, r.author, r.headline, SUBSTR(r.body, 1, 100) AS preview,
       r.date_published_parsed, p.name, p.site
FROM reviews r
JOIN products p ON r.product_id = p.id
WHERE r.rating <= 2
ORDER BY COALESCE(r.date_published_parsed, r.scraped_at) DESC
LIMIT 10
```
...
SELECT p.site,
       ...
       COUNT(r.id) AS ingested_review_rows,
       SUM(p.review_count) AS site_reported_review_total_current,
       SUM(p.review_count - COALESCE(p.ratings_only_count, 0)) AS text_review_total_current,
       ROUND(COUNT(r.id) * 100.0 / NULLIF(SUM(p.review_count), 0), 1) AS business_sample_coverage_pct,
       ROUND(COUNT(r.id) * 100.0 / NULLIF(SUM(p.review_count - COALESCE(p.ratings_only_count, 0)), 0), 1) AS crawler_text_coverage_pct
FROM products p
...
```sql
SELECT SUBSTR(COALESCE(r.date_published_parsed, r.scraped_at), 1, 10) AS publish_date,
       COUNT(*) AS negative_reviews
FROM reviews r
JOIN products p ON r.product_id = p.id
WHERE r.rating <= 2
  AND r.date_published_parsed IS NOT NULL
...
```sql
SELECT ril.label_code,
       ril.severity,
       COUNT(DISTINCT ril.review_id) AS reviews
FROM review_issue_labels ril
JOIN reviews r ON r.id = ril.review_id
JOIN products p ON p.id = r.product_id
WHERE ril.label_polarity = 'negative'
GROUP BY ril.label_code, ril.severity
...
```sql
SELECT p.name, p.site, p.ownership,
       r.rating, r.headline, r.date_published_parsed,
       ra.failure_mode, ra.impact_category, ra.insight_cn
