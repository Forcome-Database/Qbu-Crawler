...
本方案**不新增公开暴露的工作流写接口**。每日提交流程改为由 Crawler Host 上的 `qbu-crawler serve` 内嵌 scheduler 在本机触发，不通过公网 HTTP 入口触发。
...
每日 scheduler 改为 **Crawler Host 本机 `qbu-crawler serve` + 内嵌 `DailySchedulerWorker`**：

`qbu-crawler serve -> DailySchedulerWorker -> submit_daily_run()`
...
1. Crawler Host 上的 `qbu-crawler serve` 启动 `DailySchedulerWorker`
2. 到达 `DAILY_SCHEDULER_TIME` 后，scheduler 计算 `trigger_key = daily:<logical_date>`
...
4. 再把 daily-submit 切到 Crawler Host `serve` 内嵌 scheduler。
