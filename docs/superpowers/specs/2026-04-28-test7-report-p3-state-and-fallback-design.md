...
- 必须同时支持 `sqlite3.Row` 和普通 tuple row；测试/脚本未设置 `row_factory` 时不能崩溃。
- `notification_outbox.status` 中 `sent` 和历史兼容值 `delivered` 都应视为通知送达。
- `full_sent_local + deadletter_count>0 + workflow_notification_delivered=false` 是正确降级态，不应被 contract validator 判定为 delivery 冲突。
...
- manifest 能兼容未设置 `row_factory` 的普通 sqlite connection。
- outbox `sent` / `delivered` 两种历史状态都能正确推导为 delivered。
